...
function ITEM:OnUse(player, itemEntity)				
	...
	elseif (player:FindItemByID("breens_water")) then
		...
		player:TakeItem(itemTable);
		player:TakeItem(self);
	elseif (player:FindItemByID("coffee")) then
		local itemTable = player:FindItemByID("coffee");
		player:ProgressAttribute(ATB_DEXTERITY, 15, true);
		player:GiveItem(Clockwork.item:CreateInstance("roofie_c"));
		Clockwork.player:Notify(player, "You have spiked a coffee!");
		player:TakeItem(itemTable);
		player:TakeItem(self);
	else
