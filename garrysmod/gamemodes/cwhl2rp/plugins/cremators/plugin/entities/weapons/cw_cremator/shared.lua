...
--// Edited by David Ralphsky :D
--// Minor modifications added for CW compatibility by James
...
function SWEP:PrimaryAttack()
	...
	if ( SERVER && IsValid( trace.Entity ) ) then
		...
		if ( IsValid( phys ) ) then
			phys:ApplyForceOffset( self.Owner:GetAimVector() * 2.5 * phys:GetMass(), trace.HitPos )
		end
	end
 else
 self:EndSound()
...
function SWEP:IsUnderWater()
	if self:WaterLevel() < 3 then
		...
		if SERVER then
			...
			hitdie:SetKeyValue("Damage",10000)
...
function SWEP:lase2(par, stat, from, to, noise)
	if SERVER then
